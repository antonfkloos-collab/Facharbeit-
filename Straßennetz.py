"""
Straßennetz-Routing: Editor-unabhängig nutzbar als Skript oder CLI.
...
def main(
    place: str = "Siegen, Germany",
    start_query: str = "Gesamtschule Eiserfeld, Siegen, Germany",
    ziel_query: str = "Siegen ZOB, Siegen, Germany",
    ...
    try:
        start_coords = ox.geocode(start_query)
    except Exception:
        start_coords = (50.8542, 8.0248)

    try:
        ziel_coords = ox.geocode(ziel_query)
    except Exception:
    ...
    # Unfalldatei suchen: CLI > data/Standard > Fehlerhinweis
    if acc_file is None:
        acc_file = Path(__file__).with_name("data").joinpath("Unfallorte2024_LinRef.csv.csv")
    acc_file = Path(acc_file)
    if not acc_file.exists():
        raise FileNotFoundError(
            f"Unfalldatei nicht gefunden: {acc_file}. Lege sie unter 'data/' ab oder übergib --acc-file."
        )
    ...
    out_path = Path(output) if output else Path(os.path.abspath("route_eiserfeld_siegen.html"))
    m.save(out_path)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Straßennetz-Routing mit Unfalldaten")
    parser.add_argument("--place", default="Siegen, Germany", help="Ort/Region für das Straßennetz")
    parser.add_argument("--start", dest="start_query", default="Gesamtschule Eiserfeld, Siegen, Germany", help="Startadresse oder Koordinaten")
    parser.add_argument("--ziel", dest="ziel_query", default="Siegen ZOB, Siegen, Germany", help="Zieladresse oder Koordinaten")
    parser.add_argument("--acc-file", dest="acc_file", default=None, help="Pfad zur Unfalldatei (CSV)")
    parser.add_argument("--output", dest="output", default=None, help="Ausgabedatei (HTML)")
    args = parser.parse_args()
    main(
        place=args.place,
        start_query=args.start_query,
        ziel_query=args.ziel_query,
